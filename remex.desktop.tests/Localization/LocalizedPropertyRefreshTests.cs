...
public class LocalizedPropertyRefreshTests
{
    ...
    // The phone-presence line snapshots a localized string too, and is ALSO refreshed by a
    // 3-second poll — which is why it needs pinning: the staleness would be invisible today and
    // permanent the moment that poll becomes an event. Moved out of ShellViewModel to
    // PhonePresenceMonitor when the other four indicators were rebound (RemEx-0z7w, RemEx-7zzw).
    [InlineData("PhonePresenceMonitor.cs", "Refresh")]
