...
///
/// Lives in <c>Remex.Core</c> (not <c>Remex.Agent</c>) so the Windows-only WGC capture project — a
/// separate assembly that cannot reference <c>Remex.Agent</c> — shares the exact same formula rather
