// Transcribed from material-components-android 1.14.0: lib/java/com/google/android/material/color/utilities/Cam16.java
/*
...
public sealed class Cam16
{
    ...
    /// <param name="chroma">informally, colorfulness / color intensity. like saturation in HSL, except perceptually accurate.</param>
    ...
    internal static Cam16 FromIntInViewingConditions(uint argb, ViewingConditions viewingConditions)
    {
        // Transform ARGB int to XYZ
        int red = (int)((argb & 0x00ff0000u) >> 16);
        int green = (int)((argb & 0x0000ff00u) >> 8);
        int blue = (int)(argb & 0x000000ffu);
        ...
        double x, double y, double z, ViewingConditions viewingConditions)
    {
        ...
        double rA = MathUtils.SignumDouble(rD) * 400.0 * rAF / (rAF + 27.13);
        double gA = MathUtils.SignumDouble(gD) * 400.0 * gAF / (gAF + 27.13);
        double bA = MathUtils.SignumDouble(bD) * 400.0 * bAF / (bAF + 27.13);
    ...
    internal uint Viewed(ViewingConditions viewingConditions)
    {
    ...
    internal double[] XyzInViewingConditions(ViewingConditions viewingConditions, double[]? returnArray)
    {
        ...
        double rC =
            MathUtils.SignumDouble(rA) * (100.0 / viewingConditions.Fl) * Math.Pow(rCBase, 1.0 / 0.42);
        double gCBase = Math.Max(0, (27.13 * Math.Abs(gA)) / (400.0 - Math.Abs(gA)));
        double gC =
            MathUtils.SignumDouble(gA) * (100.0 / viewingConditions.Fl) * Math.Pow(gCBase, 1.0 / 0.42);
        double bCBase = Math.Max(0, (27.13 * Math.Abs(bA)) / (400.0 - Math.Abs(bA)));
        double bC =
            MathUtils.SignumDouble(bA) * (100.0 / viewingConditions.Fl) * Math.Pow(bCBase, 1.0 / 0.42);
