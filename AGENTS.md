...
- **KDE persist_mode fallback:** `SelectSources` first requests `persist_mode=PersistUntilRevoked` (+ replays a saved `restore_token`) so reconnects skip the "Share your screen" prompt. `xdg-desktop-portal-kde` rejects persistence specifically for RemoteDesktop sessions (`DBusException` with `ErrorName == "org.freedesktop.portal.Error.InvalidArgument"`, or message containing `"persist"`); on that error the call is retried once with `requestPersist=false` instead of failing the whole session. Net effect: RD capture works on all compositors, but prompt-free reconnect only works where persistence is accepted (e.g. GNOME) — KDE re-prompts every connect. Prompt-free reconnect on KDE is tracked separately and still open (`RemEx-mswt`).
