...
This project is indexed by GitNexus as **RemEx** (12327 symbols, 23275 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- `remex.agent/` — the PC side: Windows Service / Linux daemon, ASP.NET Minimal APIs, WebSocket, mDNS. Runs as LocalSystem in Session 0 on Windows.
  ...
  - `Services/WindowsActiveSession` — WTS console-session helpers for Session-0 service: `IsUserSessionAvailable` (`WTSQueryUserToken` probe), `ActiveConsoleSessionId` (`WTSGetActiveConsoleSessionId`), `TryLaunch`/`TryLaunchSessionTask` (`CreateProcessAsUser` into console session). `[SupportedOSPlatform("windows")]`. Owned here — NOT in `Remex.Core` (NativeAOT JNI) — shared by `SessionBridgingCommandService` and app launcher.
- `remex.android/` — the only client: Kotlin + Jetpack Compose + JNI → `libRemexCore.so`.
  ...
  - `ui/screens/H264StreamDecoder` — `MediaCodec` async hardware decoder (ASYNCHRONOUS mode via `setCallback(callback, Handler(HandlerThread("H264DecoderCb").looper))` — callbacks delivered on a dedicated background HandlerThread, NOT the main/UI looper; main-looper delivery starves `onInputBufferAvailable` during connection under Compose/SurfaceView/frame-routing load, causing the codec to never be fed and the stream to stay permanently black with 0 input-buffer callbacks; thread is quit via `quitSafely()` in `release()`); renders to a **SurfaceView**; bounded backlog (4 frames, drop-oldest); deferred `configure()` — does NOT configure on construction, waits for the first SPS+PPS-bearing IDR keyframe, then calls `MediaCodec.configure()` with explicit `csd-0` (SPS) / `csd-1` (PPS) before starting (device-portable; configuring with only width/height and relying on inline SPS/PPS auto-detect silently wedges some hardware decoders — no output produced, input buffers never free, black screen + keyframe-request flood); MUST NOT set `KEY_COLOR_FORMAT` (Qualcomm `c2.qti.avc.decoder` rejects `COLOR_FormatSurface` with "configureIntf failed 95 / ? is not a supported pixel format" → zero output), `KEY_LOW_LATENCY`, or `KEY_OPERATING_RATE` (these shrink the decoder's output/DPB pool to ~2 buffers → classic "Works: Q:2/Done:2 then stall" black screen in async mode with a SurfaceView); only `KEY_PRIORITY=0`, `KEY_MAX_INPUT_SIZE`, `csd-0`, `csd-1` are set (#2b); pre-keyframe P-frames dropped silently without flooding `onKeyframeNeeded`; IDR units queued with `BUFFER_FLAG_KEY_FRAME`; `onKeyframeNeeded` on backlog overflow or transient error; `onInitFailure` on non-transient decoder error; all codec state guarded by `codecLock` (callbacks on dedicated H264DecoderCb HandlerThread, `decodeFrame` on bg coroutine, `release` on main thread). (RemEx-bqc / RemEx-kx4 / RemEx-x0b / #2b)
...
- **`WindowsActiveSession` Session-0 console helpers**: Static class in `remex.agent/Services/WindowsActiveSession.cs`, `[SupportedOSPlatform("windows")]`. `IsUserSessionAvailable` — checks `WTSGetActiveConsoleSessionId` + `WTSQueryUserToken` to confirm a signed-in user session exists. `ActiveConsoleSessionId` — raw console session ID (`0xFFFFFFFF` when none). `TryLaunch`/`TryLaunchSessionTask` — spawn the host binary into the active console session via `CreateProcessAsUser` for desktop-bound actions (lock, monitor-off) that no-op in Session 0. Owned here and NOT in `Remex.Core` (NativeAOT JNI) — keeps WTS/P/Invoke out of the shared library. Shared by `SessionBridgingCommandService` and app launcher.
