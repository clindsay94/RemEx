...
- **1440p FPS bottleneck: CPU color-convert in `FFmpegH264Encoder`, not NVENC core:** Plain `h264_nvenc` appends `-pix_fmt yuv420p`, which forces ffmpeg to run CPU swscale (BGRA→YUV) on every frame — ~20 ms at 1440p → ~35 fps cap while the RTX GPU idles. Reducing capture `scale` makes it **worse** because the host downscale is also CPU (GDI+). Fix (`h264_nvenc_cuda` — pending commit + device verification): `FFmpegH264Encoder.Initialize()` tries `"h264_nvenc_cuda"` **first** in `codecsToTry`; its args use `-vf hwupload_cuda,scale_cuda=format=nv12 -c:v h264_nvenc ...` (no `-pix_fmt yuv420p`), moving BGRA→NV12 to the GPU (~1 ms). If the ffmpeg build lacks CUDA filters it fails to start and falls through to `"h264_nvenc"` (CPU path) — never slower than before. Check `ActiveCodecName` in the host log to see which path won: `"h264_nvenc_cuda"` = GPU/fast (unblocks 120 fps at full res); `"h264_nvenc"` = CPU/~35 fps cap. Note: the earlier "120fps" frame-pacing fix was OS timer floor (`PrecisionPacer` SpinWait) — unrelated to encode throughput; the color-convert was always the real 1440p cap.
