...
- `remex.android/` — the only client: Kotlin + Jetpack Compose + JNI → `libRemexCore.so`.
  ...
  - `ui/screens/H264StreamDecoder` — `MediaCodec` async hardware decoder (ASYNCHRONOUS mode via `setCallback`); bounded backlog (4 frames, drop-oldest); deferred `configure()` — does NOT configure on construction, waits for the first SPS+PPS-bearing IDR keyframe, then calls `MediaCodec.configure()` with explicit `csd-0` (SPS) / `csd-1` (PPS) before starting (device-portable; configuring with only width/height and relying on inline SPS/PPS auto-detect silently wedges some hardware decoders — no output produced, input buffers never free, black screen + keyframe-request flood); MUST NOT set `KEY_COLOR_FORMAT` for Surface-output decoders — Qualcomm `c2.qti.avc.decoder` rejects `COLOR_FormatSurface` with "configureIntf failed 95 / ? is not a supported pixel format", producing zero output (black screen + per-frame keyframe flood) (#2b); pre-keyframe P-frames dropped silently without flooding `onKeyframeNeeded`; IDR units queued with `BUFFER_FLAG_KEY_FRAME`; `onKeyframeNeeded` on backlog overflow or transient error; `onInitFailure` on non-transient decoder error; all codec state guarded by `codecLock` (callbacks on codec handler thread, `decodeFrame` on bg coroutine, `release` on main thread). (RemEx-bqc / RemEx-kx4 / RemEx-x0b / #2b)
...
- **`H264StreamDecoder` `KEY_COLOR_FORMAT` prohibition (Surface-output decoders)**: NEVER set `KEY_COLOR_FORMAT` in the `MediaFormat` when the decoder renders to a `Surface`. Qualcomm's `c2.qti.avc.decoder` rejects `COLOR_FormatSurface` (`0x7F000789`) with "configureIntf failed 95 / ? is not a supported pixel format", producing zero output — a silently black stream with a per-frame keyframe-request flood. When rendering to a Surface, the Surface dictates the output format; only `KEY_LOW_LATENCY`, `KEY_PRIORITY`, `KEY_OPERATING_RATE`, and `csd-0`/`csd-1` should be set. (#2b decode-stall, Qualcomm pixel-format reject)
