...
**Linux RD: NVENC engagement + per-monitor crop via post-capture cropping (RemEx-nadp, IN PROGRESS):** Root cause: on Wayland/dual-monitor a 5120×1440 full-virtual-desktop frame exceeds NVENC's practical fast path and the stream fell back to ~13 fps CPU MJPEG. Fix landed so far (`LinuxScreenCaptureService`, `LinuxJpegEncoder`): capture always grabs the full virtual-desktop frame; both encoders now take optional `cropX/cropY/cropW/cropH` params and, when non-zero, `SKImage.Subset` the frame to the active target's rect before scale/encode — new private `LinuxScreenCaptureService.TryGetActiveCrop(frameWidth, frameHeight, out x,y,w,h)` computes that rect from `_activeLeft/_activeTop/_activeWidth/_activeHeight` and is called from both the PipeWire raw-capture path and the JPEG path. This means per-monitor capture no longer requires the capture backend itself to target a monitor — it works on Wayland, on any display server, and survives mid-session/topology-refresh with an active PipeWire session (the old restriction — Monitor mode blocked whenever a PipeWire coordinator was running or the display server wasn't X11 — is removed). `SetCaptureCoordinator` now defaults a fresh multi-monitor PipeWire session to `DesktopCaptureMode.Monitor` on the primary display (crop to ~2560×1440) instead of `VirtualDesktop`, to stay under the 4096px H.264 limit and land on NVENC's fast path without a CPU downscale by default. New KDE/Wayland display enumeration: `TryDetectWithKScreenDoctor()` runs `kscreen-doctor -o` and parses per-output geometry via `ParseKScreenDisplays`/`KScreenGeometryRegex` — tried FIRST in `DetectScreenSize()` on Wayland, ahead of xrandr, because KWin is not a wlroots compositor (`wlr-randr` fails) and XWayland's `xrandr` view of KWin outputs is unreliable for per-monitor geometry, whereas `kscreen-doctor` talks to KScreen directly and reports true per-output geometry + the priority-1 primary. **Native PipeWire framerate negotiation:** `pipewire_capture.c` (`pw_thread_func`) also adds an explicit `SPA_FORMAT_VIDEO_maxFramerate` choice-range `[1,120]` (default 120) to the `SPA_PARAM_EnumFormat` pod, alongside the pixel-format enum — without it, KWin's ScreenCast implementation serves frames on a slow, damage-driven cadence (~12 fps observed) independent of on-screen activity or encoder speed, capping the stream regardless of the NVENC/crop fixes above. Same technique OBS and gnome-remote-desktop use to get a real framerate out of KWin. **Do not drop this param when touching the EnumFormat pod** — omitting it silently reintroduces the KWin FPS cap. **Not yet closed** — remaining scope per the bead: host-side H.264-preference hardening (prefer H.264 when encodable+decodable) and default-target=primary-monitor wiring beyond the PipeWire-session default above.
...
- **Performance presets — `applyDesktopPreset(quality, fps, scale)`** (`RemoteDesktopViewModel`): FilterChip row at the top of the Stream settings panel applies all three config values atomically (persist + push in one call). Presets: **Performance** (scale=0.5 / 120fps / q85), **Balanced** (0.75 / 90fps / q90), **Crisp** (1.0 / 120fps / q100). Always use `applyDesktopPreset` to change presets — do not set quality/fps/scale individually or the values will diverge.
...
This project is indexed by GitNexus as **RemEx** (11452 symbols, 24382 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
