...
{
    if (!out_handle) return REMEX_ERR_GENERIC;

    remex_pw_session_t *sess = calloc(1, sizeof(remex_pw_session_t));
    if (!sess) return REMEX_ERR_GENERIC;
...
int remex_pw_session_acquire_frame(
    ...
    int                       timeout_ms)
{
    if (!handle || !out_descriptor) return REMEX_ERR_GENERIC;
...
void remex_pw_session_destroy(void *handle)
{
    ...
    if (sess->pw_lib) dlclose(sess->pw_lib);
    free(sess);
}
...
{
    if (!out_buf || buf_size == 0) return REMEX_ERR_BUFFER_TOO_SMALL;

    void *pw = NULL;
    const char *pw_libs[] = { "libpipewire-0.3.so.0", "libpipewire-0.3.so", NULL };
    for (int i = 0; pw_libs[i]; i++) {
        pw = dlopen(pw_libs[i], RTLD_NOW | RTLD_LOCAL);
        if (pw) { dlclose(pw); break; }
    }
