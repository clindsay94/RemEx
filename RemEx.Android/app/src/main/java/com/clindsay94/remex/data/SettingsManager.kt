...
class SettingsManager(val context: Context) {
    ...
    companion object {
        ...
        val VERTICAL_SCROLL_SENSITIVITY_KEY = floatPreferencesKey("vertical_scroll_sensitivity")
    ...
    val mouseFabXFlow: Flow<Float> =
            context.dataStore.data.map { preferences -> preferences[MOUSE_FAB_X_KEY] ?: Float.NaN }

    val mouseFabYFlow: Flow<Float> =
            context.dataStore.data.map { preferences -> preferences[MOUSE_FAB_Y_KEY] ?: Float.NaN }
    ...
    val themeModeFlow: Flow<String> =
            context.dataStore.data.map { preferences -> preferences[THEME_MODE_KEY] ?: "system" }
    ...
    val fontFamilyFlow: Flow<String> =
            context.dataStore.data.map { preferences -> preferences[FONT_FAMILY_KEY] ?: "default" }

    val cardCornerRadiusFlow: Flow<Int> =
            context.dataStore.data.map { preferences -> preferences[CARD_CORNER_RADIUS_KEY] ?: 20 }
    ...
    suspend fun saveRemoteDesktopScrollSensitivity(vertical: Float, horizontal: Float) {
        context.dataStore.edit { preferences ->
            ...
            preferences[HORIZONTAL_SCROLL_SENSITIVITY_KEY] = horizontal.coerceIn(0.1f, 5.0f)
        }
    }

    suspend fun saveHomeLayout(layoutJson: String) {
        context.dataStore.edit { preferences -> preferences[HOME_LAYOUT_JSON_KEY] = layoutJson }
    }
