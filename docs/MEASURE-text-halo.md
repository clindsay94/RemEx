...
**Eyes pass — completed 2026-09-11** (Release build of this branch). The remainder of the pass was finished the same day; the full table with screenshot paths (`%TEMP%\remex-ui\text-jt6w5\`) lives in bead `RemEx-jt6w5.6`'s notes (`bd show RemEx-jt6w5.6` — "Eyes pass — completed by the controller"). Observed: bell/size sliders exercised at both ends for Headers and Sensor; Bold confirmed on every section; Text shadow at 0/40/100 in both Dark and Light — a dark halo behind light text, a light halo behind dark text, and no visible halo on a flat surface; the backdrop plate visible behind sensor-card titles on the canvas at 0.8 and 1.6 and on Home pinned tiles; no unstyled flash at startup (the splash screen covers the first seconds); Reset restores the default table. The pass found three defects, all fixed on this branch: clipped Bold switches (RemEx-jt6w5.10), Bold body breaking UI Automation (RemEx-jt6w5.11), and Bold thinning Black titles (RemEx-jt6w5.12). RemEx-jt6w5.9 was closed as superseded. One item from the pass is still uncaptured — Small text at 1.6 / Headers+Body at 0.8 screenshots — tracked as polish-only in RemEx-jt6w5.13.
