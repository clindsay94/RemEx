...
**Eyes pass — completed 2026-09-11** (Release build of this branch). The full table with screenshot paths (`%TEMP%\remex-ui\text-jt6w5\`) lives in bead `RemEx-jt6w5.6`'s notes (`bd show RemEx-jt6w5.6` — "Eyes pass — completed by the controller"). Observed: size sliders at max for Headers and at both ends for Sensor; Bold confirmed on every section; Text shadow at 0 and 100 in both Dark and Light over a wallpaper — a dark halo behind light text, a light halo behind dark text; the backdrop plate behind sensor-card titles on the canvas at 0.8 and 1.6, and on Home pinned tiles at 1.6 (at 0.8 it matches the dark tile and is invisible by design); no unstyled flash at startup (the splash screen covers the first seconds); Bold body persisted across a cold restart; Reset restores the default table. Not exercised by eye: the tray flyout strip, the halo on a flat (non-wallpaper) surface, and the Export → Reset → Import round trip — the last is covered by `SavefileTypographyRoundTripTests`. The pass found three defects, all fixed on this branch: clipped Bold switches (RemEx-jt6w5.10), Bold body breaking UI Automation (RemEx-jt6w5.11), and Bold thinning Black titles (RemEx-jt6w5.12). RemEx-jt6w5.9 was closed as superseded. Still uncaptured — Small text at 1.6 / Headers+Body at 0.8 screenshots — tracked as polish-only in RemEx-jt6w5.13.
