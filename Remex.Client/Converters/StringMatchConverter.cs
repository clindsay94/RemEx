...
public static class StringMatchConverter
{
    ...
    /// <summary>Returns true when the bound string equals "Glass".</summary>
    public static readonly IValueConverter IsGlass =
        new StringEqualsConverter("Glass");
    ...
    /// <summary>Returns true when the bound value's string representation equals the ConverterParameter string (case-insensitive).</summary>
    public static readonly IValueConverter EqualsParameter = new ParameterEqualsConverter();
    ...
    private sealed class ParameterEqualsConverter : IValueConverter
    {
        public object Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
            => value?.ToString() is { } s && string.Equals(s, parameter?.ToString(), StringComparison.OrdinalIgnoreCase);
